 ...
 * 
 * Modified by Amer Ghanem on Feb 8th 2012
 * methods modified
 * 
 *  MyTwitterStreamImpl(Configuration conf, Authorization auth)
 *  privateStatusStream getCountStream(String relativeUrl, int count)
 ...
 *  public void addListener(MyStatusListener listener)
    }
 * 
 */
...
public class JSONTwitterStreamImpl extends TwitterBaseImpl implements
		TwitterStream {
    ...
    /*package*/
    JSONTwitterStreamImpl(Configuration conf, Authorization auth) {
        super(conf, auth);
    ...
    private StatusStream getCountStream(String relativeUrl, int count) throws TwitterException {
        ensureAuthorizationEnabled();
        try {
            return new JSONStatusStreamImpl(getDispatcher(), http.post(conf.getStreamBaseURL() + relativeUrl
    ...
    public StatusStream getRetweetStream() throws TwitterException {
        ensureAuthorizationEnabled();
        try {
            return new JSONStatusStreamImpl(getDispatcher(), http.post(conf.getStreamBaseURL() + "statuses/retweet.json"
    ...
    public StatusStream getSampleStream() throws TwitterException {
        ensureAuthorizationEnabled();
        try {
            return new JSONStatusStreamImpl(getDispatcher(), http.get(conf.getStreamBaseURL() + "statuses/sample.json"
                    , auth), conf);
        } catch (IOException e) {
            throw new TwitterException(e);
        }
    ...
    private Dispatcher getDispatcher() {
        if (null == JSONTwitterStreamImpl.dispatcher) {
            synchronized (TwitterStreamImpl.class) {
                if (null == JSONTwitterStreamImpl.dispatcher) {
                    ...
                    JSONTwitterStreamImpl.dispatcher = new DispatcherFactory(conf).getInstance();
                }
            }
        }
        return JSONTwitterStreamImpl.dispatcher;
    }
    ...
    public StatusStream getFilterStream(FilterQuery query) throws TwitterException {
        ensureAuthorizationEnabled();
        try {
            return new JSONStatusStreamImpl(getDispatcher(), http.post(conf.getStreamBaseURL()
     ...
     * {@inheritDoc}
     */
    public void addListener(JSONStatusListener listener) {
        addListener((StreamListener) listener);
    }
    ...
    public boolean equals(Object o) {
        ...
        JSONTwitterStreamImpl that = (JSONTwitterStreamImpl) o;
