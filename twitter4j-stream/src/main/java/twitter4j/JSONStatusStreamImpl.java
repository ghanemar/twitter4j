...
/**
 * StatusStream implementation. This class will allow the class MyStatusListener to return raw stream in the form of JSON.
 * 
 * This class is slightly modified version of StatusStreamImpl authored by Yusuke Yamamoto - yusuke at mac.com for Twitter4J 2.1.2
 * 
 * Changed methods from the original form are
 * 
 * Constructors
 * protected void onStatus(JSONObject json)
 *
 * @author Amer Ghanem - amer@ghanem.ps
 * 
 */
class JSONStatusStreamImpl extends AbstractStreamImplementation implements StatusStream {
    /*package*/

    JSONStatusStreamImpl(Dispatcher dispatcher, InputStream stream, Configuration conf) throws IOException {
        super(dispatcher, stream, conf);
    }
    /*package*/

    JSONStatusStreamImpl(Dispatcher dispatcher, HttpResponse response, Configuration conf) throws IOException {
    ...
    protected void onStatus(JSONObject json) throws TwitterException {
        for (StreamListener listener : listeners) {
            ((JSONStatusListener) listener).onStatus((json));
        }
    }
